...
const P = {
  ...
  severe:["BEEF","BLACK TEA","BELL PEPPER","BRUSSELS SPROUT","CABBAGE","CANOLA OIL","CAPERS","CAULIFLOWER","CHICKPEA","CILANTRO","COFFEE","CUMIN","ENDIVE","GARLIC","GREEN TEA","HONEYDEW MELON","JALAPENO PEPPER","LOBSTER","MONK FRUIT","MULBERRY","ONION","PINTO BEAN","PISTACHIO","POPPY SEED","RICE (ALL)","SCALLION","SEA BASS","TOMATO","WAKAME SEAWEED","EGG WHITE"],
...
function detectSpikes(pts) {
  ...
  pts.forEach((p,i)=>{
    ...
    if(p.temp-b.temp>=0.45&&!spikes.find(s=>s.m==="temp"))spikes.push({min:p.min,m:"temp",label:"Temperature rise",val:`+${(p.temp-b.temp).toFixed(2)} degC`,level:p.temp-b.temp>=0.65?"severe":"moderate"});
        ...
        {!monActive&&monTimeline.length===0?(
                ...
                <div style={{background:`${T.ok}0F`,border:`1px solid ${T.ok}30`,borderRadius:7,padding:"8px 10px",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                    {monFoods.map((f,i)=>{const fu=f.toUpperCase(),isS=P.severe.some(s=>fu.includes(s)||s.includes(fu.split(" ")[0])),isM=P.moderate.some(s=>fu.includes(s)||s.includes(fu.split(" ")[0])),col=isS?T.err:isM?T.warn:T.ok;return(
                      <span key={i} onClick={()=>setMonFoods(p=>p.filter((_,j)=>j!==i))} style={{background:col+"18",border:`1px solid ${col}50`,borderRadius:4,padding:"2px 8px",fontSize:11,fontFamily:fonts.sans,color:col,cursor:"pointer"}}>{f} x</span>
                    );})}
                  </div>
                </div>
              )}
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              ...
              <MiniChart pts={visiblePts} key_="temp" color="#8A7050" label="Body Temp" unit=" degC"/>
            </div>
            {currentPt&&<Panel>
              <div style={{display:"flex",gap:24,flexWrap:"wrap"}}>
                {[["SpO2",currentPt.spo2,"%",T.rg,v=>v>=96],["HR",currentPt.hr,"bpm",T.err,v=>v<90],["HRV",currentPt.hrv,"ms",T.rg,v=>v>35],["Glucose",currentPt.glucose,"mg/dL",T.warn,v=>v<130],["Temp",currentPt.temp," degC","#8A7050",v=>v<37.1]].map(([label,val,unit,color,good])=>(
    ...
    if(tab==="protocol") return (
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
          {[{label:"SEVERE -- 9 MONTHS",color:T.err,text:"Beef, coffee, garlic, onion, tomato, all rice, black tea, cauliflower, Brussels sprout, chickpea, cilantro, bell pepper, scallion, canola oil, lobster, pistachio, poppy seed, capers, cumin, endive, honeydew, monk fruit, mulberry, wakame, jalapeno, egg white"},
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:5,flexWrap:"wrap"}}>
                    ...
                    {meal.isProtein&&<div style={{position:"relative",marginLeft:"auto"}} onClick={e=>e.stopPropagation()}>
                      <button onClick={()=>setPicker(isOpen?null:pk)} style={{background:isOpen?T.rgBg:T.w1,border:`1px solid ${isOpen?T.rg:T.w3}`,borderRadius:20,padding:"3px 10px",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
                        ...
                        <span style={{fontSize:8,color:T.w4}}>v</span>
                      </button>
                  ...
                  <div style={{marginTop:8}}>
                    <button onClick={()=>{if(isRecipeOpen){setRecipeTarget(null);setRecipeSteps(null);}else{setRecipeTarget(pk);fetchRecipeSteps(rotDay,k,meal.isProtein?selP:"",meal.base,meal.sides);}}} style={{background:"none",border:`1px solid ${isRecipeOpen?T.rg:T.w3}`,borderRadius:5,padding:"3px 10px",cursor:"pointer",fontSize:10,fontFamily:fonts.mono,color:isRecipeOpen?T.rg2:T.w4,display:"flex",alignItems:"center",gap:4,letterSpacing:"0.1em"}}>
                      <span>{isRecipeOpen?"v":">"}</span><span>{isRecipeOpen?"HIDE RECIPE":"STEP-BY-STEP RECIPE"}</span>
                    </button>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:18}}>
            ...
            <div>
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                {[["full_day","","Full Day"],["breakfast","","Breakfast"],["lunch","","Lunch"],["dinner","","Dinner"]].map(([id,em,label])=>(
    ...
    <div style={{minHeight:"100vh",background:T.w,color:T.w7,fontFamily:fonts.sans}}>
      ...
      <div style={{borderTop:`1px solid ${T.w3}`,padding:"14px 44px",display:"flex",justifyContent:"space-between",alignItems:"center",background:T.w1}}>
        <div style={{fontFamily:fonts.mono,fontSize:8,color:T.w4,letterSpacing:"0.12em"}}>
          <span style={{color:T.w6,fontWeight:500}}>meet mario</span> . MediBalans . Karlavagen 89, Stockholm
        </div>
