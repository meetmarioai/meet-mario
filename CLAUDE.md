...
- [x] Blood work routed to wrong UI tile — "Blood work / Other labs" tile was reading P.customLabs (never populated) instead of P.bloodWork. Fixed: tile now checks (P.bloodWork||[]).length > 0 and shows analyte count. Added bloodWork display panel in Labs tab (grid of markers with value/unit/status/ref range). max_tokens increased 4000→8000 in parse-lab to prevent truncation on 11-page PDFs.
- [x] Chat crashes on second message (round 2) — apiMsgs content now uses `typeof m.content === 'string' ? m.content : getMessageText(m.content)` to safely handle any non-string content stored in chatMsgs. Removed the intermediate `let content = m.content` that could leak array/undefined values.
- [x] Meals tab integrated — merged rotation calendar + meal builder. ROT[rotDay] food categories shown as tappable chips; selecting a slot activates it so chips populate that slot. 5 meal slots (breakfast/lunch/dinner/snack1/snack2) each generate a full recipe via Claude using patient ALCAT, CMA, genomics, and cuisine preference. Log this meal → opens existing post-meal response form. Dynamic intention line adapts per day + APOE4/GSTP1/MTHFR variants. parse-lab now logs raw Claude response text + stop_reason for Unilabs debug.
