// ── MediBalans Email Templates ───────────────────────────────────────────────
// 5-step onboarding → ALCAT conversion sequence
// Sender rotation: Mario (clinical), Christina (follow-up), info (admin)
...
<div style="background:#FAF8F4;padding:40px 48px 24px;border-bottom:1px solid #E8E0D4;">
  <div style="font-family:'Georgia',serif;font-size:22px;letter-spacing:0.04em;color:#2C2C2C;">
    meet <span style="color:#C4887A;">&#9673;</span> mario
  </div>
  <div style="font-family:'Georgia',serif;font-size:13px;color:#9A7240;margin-top:4px;letter-spacing:0.06em;">
    MediBalans AB &mdash; Precision Medicine
  </div>
...
const FOOTER = `
<div style="background:#F2EDE6;padding:32px 48px;margin-top:48px;border-top:1px solid #E8E0D4;">
  <div style="font-size:12px;color:#999;line-height:1.8;">
    MediBalans AB &bull; Stockholm, Sweden<br>
    ...
    You are receiving this because you enrolled in the MediBalans clinical programme.
...
const P = (text) => `<p style="font-size:15px;line-height:1.8;color:#2C2C2C;margin:0 0 18px;">${text}</p>`
const H2 = (text) => `<h2 style="font-family:'Georgia',serif;font-size:20px;color:#2C2C2C;margin:0 0 20px;font-weight:normal;">${text}</h2>`
const GOLD_BOX = (content) => `
<div style="background:#FDF6EC;border-left:3px solid #9A7240;padding:20px 24px;margin:28px 0;border-radius:2px;">
  <div style="font-size:14px;line-height:1.8;color:#5C4A2A;">${content}</div>
</div>
`
const CTA = (text, url) => `
<div style="margin:36px 0;">
  <a href="${url}" style="background:#9A7240;color:#FAF8F4;text-decoration:none;padding:14px 32px;font-size:14px;letter-spacing:0.06em;font-family:'Georgia',serif;display:inline-block;border-radius:2px;">
    ${text}
  </a>
</div>
`
const FOOD_LIST = (foods) => foods.map(f =>
  `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #E8E0D4;font-size:13px;">
    <span style="color:#2C2C2C;">${f.name}</span>
    <span style="color:#C4887A;font-size:12px;">${f.severe}% severe reactivity &bull; n=${f.n}</span>
  </div>`
).join('')

// ── Template 0: Day 0 — Welcome + Statistical Profile ────────────────────────
export function template0({ name, predictedReactors, symptomProfile }) {
  const topFoods = (predictedReactors || []).slice(0, 5)
  const symptoms = symptomProfile?.symptoms || []

  return {
    subject: `Your MediBalans clinical profile is ready, ${name.split(' ')[0]}`,
    sender: { name: 'Dr Mario Anthis, MediBalans', email: 'marios.anthis@medibalans.com' },
    html: `<!DOCTYPE html><html><body style="${BASE_STYLE}">
      ${HEADER('Day 0 &mdash; Your Clinical Profile')}
      ${BODY(`
        ${H2(`Welcome to MediBalans, ${name.split(' ')[0]}.`)}
        ...
        ${P(`Based on your reported symptoms${symptoms.length > 0 ? ` — ${symptoms.join(', ')} —` : ''} and demographic profile, here are the foods most likely to be driving your immune burden:`)}
        ${topFoods.length > 0 ? `
        <div style="margin:24px 0;">
          <div style="font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#9A7240;margin-bottom:12px;">Predicted high-reactivity foods</div>
          ${FOOD_LIST(topFoods)}
          <div style="font-size:11px;color:#999;margin-top:8px;">Source: MediBalans ALCAT population database</div>
        </div>` : ''}
        ...
        ${P(`Starting today, I recommend beginning the universal 21-day anti-inflammatory protocol. Eliminate seed oils, sugar, yeast, and dairy immediately. This prepares your immune system and will show results within two weeks &mdash; regardless of your ALCAT status.`)}
...
// ── Template 1: Day 2 — Protocol Check-in ────────────────────────────────────
export function template1({ name }) {
  return {
    subject: `Day 2 — how are you feeling?`,
    sender: { name: 'Christina Biri, MediBalans', email: 'christina.biri@medibalans.com' },
    html: `<!DOCTYPE html><html><body style="${BASE_STYLE}">
      ${HEADER('Day 2 &mdash; Protocol Check-in')}
      ${BODY(`
        ${H2(`${name.split(' ')[0]}, checking in.`)}
...
// ── Template 2: Day 7 — First Results + The Gap ──────────────────────────────
export function template2({ name, predictedReactors }) {
  const topFood = predictedReactors?.[0]
  return {
    subject: `One week in — your results so far`,
    sender: { name: 'Dr Mario Anthis, MediBalans', email: 'marios.anthis@medibalans.com' },
    html: `<!DOCTYPE html><html><body style="${BASE_STYLE}">
      ${HEADER('Day 7 &mdash; First Results')}
      ${BODY(`
        ${H2(`One week.`)}
...
// ── Template 3: Day 14 — Population Data Reveal + Strong CTA ────────────────
export function template3({ name, predictedReactors }) {
  const foods = (predictedReactors || []).slice(0, 8)
  return {
    subject: `What 1,042 patients tell us about your biology`,
    sender: { name: 'Dr Mario Anthis, MediBalans', email: 'marios.anthis@medibalans.com' },
    html: `<!DOCTYPE html><html><body style="${BASE_STYLE}">
      ${HEADER('Day 14 &mdash; Your Population Report')}
      ${BODY(`
        ${H2(`Two weeks. Time to go deeper.`)}
        ...
        ${foods.length > 0 ? `
        <div style="margin:24px 0;">
          <div style="font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#9A7240;margin-bottom:12px;">Your predicted reactivity profile</div>
          ${FOOD_LIST(foods)}
        </div>` : ''}
...
// ── Template 4: Day 21 — Protocol Complete + Final CTA ──────────────────────
export function template4({ name }) {
  return {
    subject: `You completed the 21-day protocol. What comes next.`,
    sender: { name: 'Christina Biri, MediBalans', email: 'christina.biri@medibalans.com' },
    html: `<!DOCTYPE html><html><body style="${BASE_STYLE}">
      ${HEADER('Day 21 &mdash; Protocol Complete')}
      ${BODY(`
        ${H2(`${name.split(' ')[0]}, you did the hard part.`)}
...
// ── Sequence schedule (days from enrolment) ──────────────────────────────────
