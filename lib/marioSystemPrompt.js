...
${genomicSnps.length > 0 ? `Genomic variants (${genomicSnps.length} SNPs): ${genomicSnps.map(s => `${s.gene} ${s.rsid} ${s.genotype} [${s.status === 'risk' ? 'HOMOZYGOUS' : 'HETEROZYGOUS'}]`).join(' | ')}` : ''}
...
${genomicSnps.map(s => {
  ...
  return `• ${s.gene} (${s.rsid}) — ${s.genotype} [${zygosity}]: ${interp}`;
